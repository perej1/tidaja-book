...
Ensimmäinen askel on **datan lukeminen**. Suhteellisen pienet aineistot säilytetään yleensä tekstitiedostoissa^[Vastaavasti suuret aineistot säilytetään usein tietokannoissa. Tällöin haluttu havaintojoukko haetaan esim. SQL-kyselyllä.]. Tässä tapauksessa aineisto on tallennettu tekstitiedostoon `iris.csv`, jonka arvot ollaan erotettu pilkuilla (eng: *comma-separated values*, CSV). Tiedoston `iris.csv` tarkemman rakenteen voit nähdä avaamalla sen valitsemassasi tekstieditorissa. Kuvan \@ref(fig:vim) näkyy, että ensimmäinen rivi koostuu sarakkeiden otsikoista ja seuraavat rivit vastaavat havaintoja. Sarakkeet taas vastaavat muuttujia, esimerkiksi viimeinen sarake antaa kurjenmiekkalajin nimen.
...
Kun työhakemisto on on asetettu, niin komennon `read.table()` avulla voit lukea datan. Komento ottaa sisäänsä useita argumentteja^[Tietojenkäsittelytieteessä *argumentti* tarkoittaa funktion syötettä (eng: *input*). Vrt. $x$ ja $f(x)$.] kuten `file` (polku datatiedostoon), `header` (sisältääkö ensimmäinen rivi sarakkeiden otsikot?), `sep` (erotin arvojen välillä), ... Jos tiedosto `iris.csv` on tallennettu työhakemiston sisällä olevaan kansioon `data`, niin havaintoaineisto voidaan lukea seuraavasti

```{r}
iris <- read.table("data/iris.csv", header = TRUE, sep = ",")
```

Kun havaintoaineisto on luettu, niin kannattaa tarkistaa että se on luettu oikein. Esimerkiksi jos erotin arvojen välillä on asetettu väärin, niin muuttujaan `iris` tallennettu havaintoaineisto tulee näyttämään oudolta. Tarksitamiseen on monta eri tapaa. Esimerkiksi kirjoittamalla muutttujan `iris` R-konsolille koko datajoukko tulostuu (paitsi jos rivejä on liikaa). Toinen tapa on `head` komento, joka tulostaa havaintoaineiston ensimmäiset rivit.
```{r}
head(iris)
```
...
Myös `str()` tulostaa ensimmäiset havainnot sarakkeittain. Lisäksi komento antaa kolumnien tietotyypit. Tässä tapauksessa ensimmäiset neljä kolumnia ovat tyyppiä `numerical` (kokonais- ja desimaaliluvut). Viimeinen kolumni on tietotyyppiä `character`, joka on tarkoitettu tekstimuotoisille muuttujille. RStudiossa luetun hvaintoaineiston näet myös käyttämällä komentoa `View()`^[`View()` toimii ainostaan RStudiossa.] (eli `View(iris)`).
...
Nyt olemme valmiita itse **data-analyysiin**. Ennen tilastollisia menetelmiä on hyödyllistä tarkastella dataa visuaalisesti. Alla symbolin `$` avulla voidaan poimia haluttuja sarakkeita nimien perusteella.

```{r base-scatter, fig.cap="Hajontakuvio halutuista muuttujista."}
plot(iris_filter$sepal_width, iris_filter$sepal_length,
     xlab = "Sepal Width", ylab = "Sepal Length")
```

Kuvan \@ref(fig:base-scatter) perusteella näyttää, että muuttujien välillä on lineaarinen riippuvuus. Tämän vuoksi lineaarinen regressiomallin

$$
  \mathrm{Sepal\ Length}= \beta\cdot \mathrm{Sepal\ Width} + \varepsilon_i
$$
...
Eli vastemuuttuja tulee symbolin `~` vasemmalle puolelle ja selitettävät muuttujat oikealle puolelle. Argumentti `data` ottaa sisäänsä muuttujan, joka sisältää havaintoaineiston.
...
Ensimmäinen komento piirtää saman hajontakuvion kuin kuvassa \@ref(fig:base-scatter). Toinen komento piirtää sovitetun regresiokäyrän. Argumetti `a` ottaa sisään vakiotermin estimaatin ja parametri `b` ottaa sisään kulmakäyrän estimaatin. Kyseiset estimaatit voidaan myös tulostaa:
```{r}
fit_lm$coefficients
```
...
Lisää tietoa kyseisestä tietotyypistä löydät esimerkiksi [täältä](https://bookdown.org/rdpeng/rprogdatascience/r-nuts-and-bolts.html#factors) perus-R näkökulmasta ja [tuolta](https://r4ds.hadley.nz/factors.html) Tidyversen näkökulmasta.
