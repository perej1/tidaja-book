...
Havaintoaineiston lukemisen jälkeen kannattaa tarkistaa, että se on luettu oikein. Esimerkiksi jos erotin arvojen välillä on asetettu väärin, niin muuttujaan `iris` tallennettu havaintoaineisto tulee näyttämään oudolta. Tarkistamiseen on monta eri tapaa. Esimerkiksi kirjoittamalla muutttujan `iris` R-konsolille koko datajoukko tulostuu (lukuunottamatta viimeisiä, jos tiedosto on liian suuri). Toinen tapa nähdä havintoaineisto on `head` komento, joka tulostaa havaintoaineiston ensimmäiset rivit.
```{r}
head(iris)
```
...
Myös `str()` tulostaa ensimmäiset havainnot sarakkeittain. Lisäksi komento antaa kolumnien tietotyypit. Tässä tapauksessa ensimmäiset neljä kolumnia ovat tyyppiä `numerical`, joka on varattu kokonais- ja desimaaliluvuille. Viimeinen kolumni on tietotyyppiä `character`, joka on tarkoitettu tekstimuotoisille muuttujille. Luetun havaintoaineiston näet myös erillisessä ikkunassa käyttäen komentoa `View()` eli `View(iris)`.
...
Nyt olemme valmiita itse **data-analyysiin**. Ennen tilastollisia menetelmiä on hyödyllistä tarkastella dataa visuaalisesti. Alla symbolin `$` avulla voidaan poimia haluttuja sarakkeita nimien perusteella. Valituista sarakkeista piirretään hajontakuvio funktion `plot` avulla.
...
::: {.example}
Esimerkiksi pseudokoodin^[Pseudokoodi on eräänlaista "leikkikoodia", jonka tarkoitus on havainnollistaa algoritmiin sisältyviä operaatioita. Itse pseudokoodi itsessään on harvoin toimivaa suoraan.]
```
data_transform <- data %>%
  f() %>%
  g() %>%
