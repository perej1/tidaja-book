...
::: {.example}
Esimerkiksi pseudokoodin^[Pseudokoodi on eräänlaista "leikkikoodia", jonka tarkoitus on havainnollistaa algoritmiin sisältyviä operaatioita. Itse pseudokoodi itsessään harvoin toimivaa.]
```
data_transform <- data %>%
  f() %>%
  g() %>%
...
Regressioviivan voimme tuottaa kuvaan \@ref(fig:ggplot-scatter) kätevästi vain lisäämällä uuden kerroksen. Estimaatit regressiokertoimille kuvaa \@ref(fig:ggplot-regression) saamme edellisen luvun muuttujasta `fit_lm`.
