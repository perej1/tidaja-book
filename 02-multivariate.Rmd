...
- Heitetään noppaa viisi kertaa peräkkäin ($\boldsymbol X_i = \mathrm{Heiton}\ i\ \mathrm{tulos}$, $i\in\{1,2,3,4,5\}$). Tällöin $(X_1, X_2, X_3, X_4, X_5)^T$ on $5$-ulotteinen diskreetti satunnaisvektori, jonka arvojoukko on $\{(j_1, j_2, j_3, j_4, j_5) : j_i\in\{1,2,3,4,5,6\}\}$.
:::
...
Vastaavasti, jos joku mielivaltainen funktio $g:\mathbb{Z}^d\to[0,1]$ toteuttaa ehdot 1 ja 2, niin se on jonkun diskreetin satunnaisvektorin pistetodennäköisyysfunktio.
:::
...
Olkoon $\boldsymbol X = (X_1, X_2, \ldots, X_d)^T$ satunnaisvektori, jolla on pistetodennäköisyysfunktio $f$. Pistetodennäköisyys- ja kertymäfunktiolla on seuraava yhteys. Kaikille $\boldsymbol x = (x_1, x_2, \ldots, x_d)\in\mathbb{R}^d$ pätee 
\begin{equation*}
  \begin{split}
...
Faktan \@ref(thm:discretecdf) kaavat (esim. kertymäfunktio näyttävät siis hyvin samankaltaiselta kuin yksiulotteiset vastineet luvun 2 alussa -- yhden summan sijasta on $d$ summaa.

### Jatkuva satunnaisvektori {-}

Tällä kurssilla moniulotteisia jatkuvia jakaumia ei käydä yhtä tarkasti kuin diskreettejä. Syy tähän on se, että
