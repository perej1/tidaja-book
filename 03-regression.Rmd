...
a) Jäännöskuvion pisteet ovat jakautuneet tasaisesti kuvaajaan.
    ...
    $\rightarrow$ Satunnaiset virhetermit ovat riippumattomia ja samoin jakautuneita. Vain residuaalien normaalijakautuneisuus tulee tarkistaa.
...
d) Jäännöskuvion pisteet ovat jakautuneet tasaisesti $x$-akselin molemmin puolin mutta pisteiden hajonta kasvaa luettaessa kuvaajaa vasemmalta oikealle (viuhkamainen muoto) .
    ...
    $\rightarrow$ Tämä voi olla merkki satunnaisvirheiden erivarianssisuudesta (heteroskedastisuus).
...
Jos joku $\mathrm{VIF}_j$:n arvo on korkea, niin selittäjiä täytyy poistaa tai niiden vaikutusta täytyy vaimentaa esimerkiksi [harjanneregression](https://en.wikipedia.org/wiki/Ridge_regression) (eng: *ridge regression*) avulla. Tähän liittyen muuttujavalintaa (eng: *variable selection*) käydään lyhyesti kurssiviikolla 11.
...
- Jos estimoimme $\hat\beta_j = 0.000001$, niin voimmeko olla suhteellisen varmoja siitä, että itseasiassa $\beta_j \neq 0$?
...
\end{equation*}
kun regressiomallissa on $d$ selittävää muuttujaa ja otoskoko on $n$. Varianssin estimaattori $\sigma^2$ itsessään mittaa mallin sopivuutta. Esimerkiksi jos $s^2$ on pieni, niin ennustuksiin liittyy vähän epävarmuutta (tämä väite kvantifioidaan tarkemmin ennustevälien yhteydessä). Lisäksi estimoidun varianssin $s^2$ avulla saadaan estimoitua jokaista estimaattoria $\hat\beta_j$ vastaava varianssi $\mathrm{Var}\left(\hat\beta_j\right)$. Esimaattori varianssille $\mathrm{Var}\left(\hat\beta_j\right)$ saadaan kaavasta
\begin{equation}
  (\#eq:se)
...
\end{equation*}
jossa $\hat\beta_0 = 50$, $\hat\beta_1 = 8$ ja $\hat\beta_1 = -3$. Estimaatteja voimme tulkita tuttuu tapaan:
...
\begin{align*}
  ...
  &H_1: \beta_j \neq 0.
\end{align*}
Nollahypoteesi $H_0$ tarkoittaa, että muuttujalla $X_j$ ei ole juurikaan tekemistä mallinnettavan ilmiön kannalta. Tällöin tilastotieteilijöiden jargonilla voisimme sanoa, että *"selittäjä ei ole tilastollisesti merkitsevä"*. Vastaavasti vastahypoteesi $H_1$ tarkoittaa, että muuttuja $X_j$ on merkityksellinen mallinnettavan ilmiön kannalta eli *"selittäjä on tilastollisesti merkitsevä"*. 

Määritellään testisuure
\begin{equation*}
  t = \frac{\hat\beta_j}{\sqrt{s^2_{\hat\beta_j}}},
\end{equation*}
jossa $\hat\beta_j$ on estimoitu regressiokerroin ja $s^2_{\hat\beta_j}$ on kaavan \@ref(eq:se) mukainen estimaattori varianssille $\mathrm{Var}\left(\hat\beta_j\right)$. Olettaessa nollahypoteesin olevan tosi tiedämme, että testisuure $t$ noudattaa $t$-jakaumaa vapausasteilla $n-d-1$ eli
\begin{equation*}
  t\sim t(n-d-1),
\end{equation*}
jossa $n$ on otoskoko ja $d$ on selittäjien määrä. Tällöin $p$-arvo lasketaan kuten missä tahansa tilastollisessa testissä -- nollahypoteesi hylätään, jos testisuureen arvo on tarpeeksi poikkeava suhteessa siihen, mitä nollahypoteesin alla voisi olettaa. Havainnollistamme t-testin suorittamista esimerkin kautta.

(ref:t-dist) Havainnekuva esimerkin $p$-arvon laskemisesta. Musta käyrä kuvaa $t$-jakauman tiheysfunktiota $f_t$  vapausasteilla 10. Vaikka $t$-jakauman muoto on samannäköinen kuin normaalijakauman, niin $t$-jakauman tiheysfunktiolla on paksummat hännät -- mitä vähemmän vapausasteita sitä paksummat hännät vastaavalla $t$-jakaumalla on. Esimerkin $p$-arvo saadaan laskettua kaavasta $\int_{-\infty}^{-4.2} f_t(x) \,\mathrm{d}t + \int_{4.2}^{\infty} f_t(x) \,\mathrm{d}t = 2\int_{-\infty}^{-4.2} f_t(x) \,\mathrm{d}t = 2F_t(-4.2)$.
...
Tilastollisia testejä ei kuitenkaan tarvitse suorittaa manuaalisesti. R funktio `summary()` antaa virhetermien varianssin estimaatin $s^2$, estimaatit $s_{\hat\beta_j}^2$ ja kertoimia $\beta_j$ vastaavia t-testien $p$-arvot.
...
Esimerkiksi `Residual standard error` antaa $s^2 = 3.09$, jonka avulla ollaan laskettu $s_{\hat\beta_0}\approx 5.20$, $s_{\hat\beta_1}\approx 0.53$ ja $s_{\hat\beta_2}\approx 0.91$ (katso sarake `Std. Error`). Regressiokertoimia $\beta_0$, $\beta_1$ ja $\beta_2$ vastaavien merkitsevyystestien (t-testi) $p$-arvot ovat $\approx 0.30$, $\approx 0.093$ ja $\approx 0.00073$ (sarake `Pr(>|t|)`). Olkoon merkitsevyystaso $\alpha = 0.05$. Näemme siis, että molemmat selittäjät `advertisement` ja `price` ovat tilastollisesti merkittäviä. Toisaalta vakiotermiin $\beta_0$ liittyvää nollahypoteesia $\beta_0 = 0$ ei hylätä. Datan perusteella meillä ei siis ole tarpeeksi todisteita siitä, että vakiotermi olisi erisuuri kuin nolla. Tätä kuvaa myös vakiotermin estimaattiin liittyvä suuri hajonta $s_{\hat\beta_0}\approx 5.20$.
:::
