...
\begin{equation}
  (\#eq:regression)
  \begin{split}
    y_i &= \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \beta_d x_{id} + \varepsilon_i \\
...
1. jokainen satunnaismuuttuja $\varepsilon_i$ noudattaa normaalijakaumaa $\varepsilon_i\sim N(0, \sigma^2)$ (havainnosta $i$ riippumattomilla) odotusarvolla $0$ ja varianssilla $\sigma^2 > 0$, ja
...
Tilastollisessa mallintamisessa on aina tärkeää miettiä ennen itse analyysiä, pätevätkö mallin oletukset. Tätä kutsutaan mallidiagnostiikaksi, jota käsitellään tarkemmin luvussa \@ref(sec:diagnostic).
...
\begin{split}
  ...
  \end{split}
\end{equation*}
Eli jos ei oteta huomioon ensimmäistä saraketta, niin matriisin $\boldsymbol X$ sarake $j$ kuvaa havaintoja selitettävästä muuttujasta $X_j$, $j\in\{1,\ldots, d\}$. Matriisin $\boldsymbol X$ ensimmäinen sarake ykkösiä ottaa huomioon sen, että parametri $\beta_0$ otetaan huomioon oikealla tavalla suoritettaessa matriisikertolaskut. Yhtälön \@ref(eq:regression) määritelmä voidaan siis kirjoittaa matriisikertolaskuilla muodossa
\begin{equation}
(\#eq:regmatrix)
...
::: {.example #regconstruction name="Esimerkki"}
